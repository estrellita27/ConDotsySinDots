...
namespace SML.Dots
{
    public class Romano : MonoBehaviour
    {
        ...
        private const float destructionTime = 100000000f; // Time after which the entity is destroyed
        ...
        public void Update()
        {
            ...
            if (Random.value < 0.0009f)
            {
                Destroy(gameObject);
            }

            transform.localPosition += Direction * Time.deltaTime* 60f;
        }
    }
}